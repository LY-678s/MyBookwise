...
fun AccountWalletScreen(
    ...
    Scaffold(
        ...
        Box(Modifier.fillMaxSize().padding(padding)) {
            ...
            if (showLevelGuide) {
                AlertDialog(
                    onDismissRequest = { showLevelGuide = false },
                    title = { Text("会员等级说明") },
                    text = {
                        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                            Text(
                                "开通会员后从 1 级起步；积分按消费 1:1 累计，达到阈值自动升级。",
                                style = MaterialTheme.typography.bodySmall,
                                color = MaterialTheme.colorScheme.outline
                            )
                            state.memberLevelGuide.forEach { row ->
                    ...
                    confirmButton = {
                        TextButton(onClick = { showLevelGuide = false }) { Text("关闭") }
                    }
                )
            }
        }
